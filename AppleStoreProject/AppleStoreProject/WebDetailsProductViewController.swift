...
final class WebDetailsProductViewController: UIViewController {
    ...
    private func createToolBarButtons() {
        backButton = UIBarButtonItem(image: UIImage(systemName: "chevron.backward.2"),
                                     style: .plain, target: self, action: #selector(goBack))
        forwardButton = UIBarButtonItem(image: UIImage(systemName: "chevron.forward.2"),
                                        style: .plain, target: self, action: #selector(goForward))
        refreshButton = UIBarButtonItem(image: UIImage(systemName: "arrow.clockwise"),
    ...
    private func selectUrlByImage() {
        ...
        if let myUrl = URL(string: productUrl) {
